...
# VERSION  : 1.0.1
# CHANGED  : 10-06-2026 -- PrinterName support: uses $map.PrinterName when set for Devices/PrinterPorts/default printer name; falls back to prior behaviour when blank
...
function Update-VBUserPrinterRegistry {
    ...
    .PARAMETER Mappings
        Array of PSCustomObjects, each with OldPath, NewPath, DriverName, and PrinterName properties.
        Produced by Set-VBUserPrinterMigration from CSV or hashtable input.
        PrinterName is optional -- when set it is used as the display name for IP destinations.
    ...
    param(
        ...
        [Parameter(Mandatory)]
        [object[]]$Mappings   # PSCustomObjects: OldPath, NewPath, DriverName, PrinterName
    )
    ...
    foreach ($map in $Mappings) {
        ...
        # Explicit PrinterName on the mapping rule takes precedence for IP destinations;
        # falls back to UNC share segment or existing device name when not supplied.
        $newDisplayName = $null
        if ($isNewUNC) {
            $newDisplayName = $newPath
        }
        elseif ($map.PrinterName) {
            # Explicit friendly name supplied -- use it regardless of old path type
            $newDisplayName = $map.PrinterName
        }
        elseif ($isOldUNC) {
            # UNC -> IP fallback: use share name (e.g. \\server\HP01 -> HP01)
